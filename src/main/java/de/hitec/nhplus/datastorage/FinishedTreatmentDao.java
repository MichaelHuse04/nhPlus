 ...
 */
public class FinishedTreatmentDao extends DaoImp<Treatment> {
     ...
     */
    public FinishedTreatmentDao(Connection connection) {
        super(connection);
    }
    ...
    protected PreparedStatement getCreateStatement(Treatment treatment) {
        ...
        try {
            final String SQL = "INSERT INTO finished_treatment (pid, cid, treatment_date, begin, end, description, remark) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    protected PreparedStatement getReadByIDStatement(long tid) {
        PreparedStatement preparedStatement = null;
        try {
            final String SQL = "SELECT * FROM finished_treatment WHERE tid = ?";
            preparedStatement = this.connection.prepareStatement(SQL);
    ...
    protected PreparedStatement getReadAllStatement() {
        ...
        try {
            final String SQL = "SELECT * FROM finished_treatment";
     ...
     */
    private PreparedStatement getReadAllFinishedTreatmentsOfOnePatientByPid(long pid) {
        PreparedStatement preparedStatement = null;
        try {
            final String SQL = "SELECT * FROM finished_treatment WHERE pid = ?";
            preparedStatement = this.connection.prepareStatement(SQL);
     ...
     * <code>ResultSet</code>.
     */
    public List<Treatment> readFinishedTreatmentsByPid(long pid) throws SQLException {
        ResultSet result = getReadAllFinishedTreatmentsOfOnePatientByPid(pid).executeQuery();
    ...
    protected PreparedStatement getDeleteStatement(long tid) {
        ...
        try {
            final String SQL =
                    "DELETE FROM finished_treatment WHERE tid = ?";
            preparedStatement = this.connection.prepareStatement(SQL);
            preparedStatement.setLong(1, tid);
        } catch (SQLException exception) {
            exception.printStackTrace();
